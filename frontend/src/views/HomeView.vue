  ...
  <div class="max-w-4xl mx-auto">
    ...
    <div v-if="errorMessage" class="mt-8 card border-accent-error">
      <div class="flex items-start space-x-3">
        ...
        <div>
          <h3 class="font-semibold text-accent-error mb-1">{{ $t('home.errorTitle') }}</h3>
    ...
    <div class="mt-16 grid grid-cols-1 md:grid-cols-3 gap-6">
      <div class="card text-center">
        ...
        <h3 class="font-semibold mb-2">{{ $t('home.feature1Title') }}</h3>
        <p class="text-sm text-theme-muted">{{ $t('home.feature1Desc') }}</p>
      </div>
      ...
      <div class="card text-center">
        ...
        <h3 class="font-semibold mb-2">{{ $t('home.feature2Title') }}</h3>
        <p class="text-sm text-theme-muted">{{ $t('home.feature2Desc') }}</p>
      </div>
      ...
      <div class="card text-center">
        ...
        <h3 class="font-semibold mb-2">{{ $t('home.feature3Title') }}</h3>
        <p class="text-sm text-theme-muted">{{ $t('home.feature3Desc') }}</p>
      </div>
    </div>
  </div>
</template>
...
const handleFileUploaded = async (fileData) => {
  ...
  statusMessage.value = t('home.convertingStatus')
  ...
  try {
    ...
    if (response.data.success) {
      ...
      statusMessage.value = t('home.conversionSuccess')
      downloadUrl.value = response.data.data.downloadUrl
      outputFilename.value = response.data.data.outputFilename
    } else {
      throw new Error(response.data.message || t('home.conversionError'))
    }
  } catch (error) {
    conversionStatus.value = 'error'
    statusMessage.value = t('home.conversionError')
    errorMessage.value = error.response?.data?.message || error.message || t('home.conversionError')
  }
}
