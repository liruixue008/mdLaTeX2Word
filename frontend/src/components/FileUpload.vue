<template>
  <div class="card">
    <div class="text-center mb-6">
      <h3 class="text-xl font-semibold mb-2">{{ $t('upload.title') }}</h3>
      <p class="text-sm text-theme-muted">{{ $t('upload.subtitle') }}</p>
    </div>
    ...
    >
      <div class="flex flex-col items-center space-y-4">
        ...
        <div>
          ...
          <p class="text-sm text-theme-muted">{{ $t('upload.browse') }}</p>
        </div>
    ...
    <div v-if="isUploading" class="mt-6">
      <div class="flex items-center justify-center space-x-3">
        ...
        <span class="text-theme-muted">{{ $t('upload.uploading') }}</span>
      </div>
...
const validateAndSetFile = (file) => {
  ...
  if (!isValidExtension) {
    emit('upload-error', t('upload.errorType'))
    return
  }
  ...
  if (file.size > maxSize) {
    emit('upload-error', t('upload.errorSize'))
    return
  }
...
const uploadFile = async () => {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      throw new Error(response.data.message || t('upload.errorUpload'))
    }
  } catch (error) {
    emit('upload-error', error.response?.data?.message || error.message || t('upload.errorUpload'))
    clearFile()
  } finally {
